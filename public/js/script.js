...
function renderTrending(items) {
  ...
  el.innerHTML = items
    .map((t, i) => {
        ...
        <div class="thumb" style="width:52px;height:52px;">
          ${
            t.image
              ...
              : `<div class="no-image" style="font-size:9px;">${t.title.slice(0, 2)}</div>`
          }
        </div>
      ...
      return t.id
        ? `<li>
            <a href="/movie/${t.id}" style="display:flex;gap:12px;align-items:flex-start;">
              ${inner}
            </a>
          </li>`
        : `<li>${inner}</li>`;
...
async function renderHero() {
  try {
    ...
    const excerpt = document.querySelector("#hero-content p");
    const meta = document.querySelector("#hero-content .meta-row");
...
function renderGenreGrid(genre) {
  const filtered = allMoviesForGenres.filter(m =>
  ...
  if (!filtered.length) {
    ...
    if (genreGrid) {
      genreGrid.innerHTML =
        `<p style="color:var(--ink-soft);grid-column:1/-1;padding:20px 0;">
          No ${genre} movies yet — check back after the next sync.
        </p>`;
    }
  }
}
...
async function renderGenreSection() {
  allMoviesForGenres =
    await fetchPosts("movie", 100);

  const tabsEl =
    document.getElementById("genre-tabs");
...
function initNewsletter() {
  ...
  const note =
    document.getElementById("newsletter-note");
...
document.addEventListener(
  ...
  async () => {
    ...
    renderReviews(
      await fetchPosts("review", 4)
    );

    renderMovies(
      await fetchPosts("movie", 6)
    );

    renderArticles(
      await fetchPosts("article", 4)
    );

    renderTrending(
      await fetchPosts("trending", 5)
    );

    await renderTop10();

    await renderGenreSection();
  }
);
