...
async function fetchTmdbExtras(env, tmdbId) {
        ...
        try {
          ...
          if (backdropRes.ok) {
            ...
            await env.IMAGES.put(bKey, await backdropRes.arrayBuffer(), {
              httpMetadata: {
                contentType: backdropRes.headers.get("Content-Type") || "image/jpeg"
              },
            });
  ...
  // region-aware (powered by JustWatch). We default to the US region;
  // the returned "link" already points at a page showing real availability,
  ...
  try {
    ...
    if (posterRes.ok) {
      ...
      await env.IMAGES.put(key, await posterRes.arrayBuffer(), {
        httpMetadata: {
          contentType: posterRes.headers.get("Content-Type") || "image/jpeg"
        },
      });
...
// whichever movie is #1 on TMDB's trending list right now.
// Featured posts intentionally do NOT store tmdb_id because tmdb_id is UNIQUE
// and the same movie may already exist as a normal movie post.
...
async function importTrendingFromTMDB(env) {
  ...
  const movies = (listData.results || []).slice(
    0,
    TMDB_IMPORT_LIMIT * 3
  );
  ...
  for (const movie of movies) {
    ...
    const existing = await env.DB.prepare(
      "SELECT id FROM posts WHERE tmdb_id = ?"
    ).bind(movie.id).first();
  ...
  // Featured post does not use tmdb_id because tmdb_id is UNIQUE.
...
function rewriteCleanUrl(request, url) {
  ...
  if (genreMatch) {
    ...
    const target = new URL(
      `/coming-soon.html?section=${encodeURIComponent(name)}`,
      url.origin
    );
...
async function handleApi(request, env, url) {
  ...
  if (pathname === "/api/login" && request.method === "POST") {
    ...
    if (!env.ADMIN_PASSWORD) {
      return json(
        { error: "Server is missing ADMIN_PASSWORD secret." },
        { status: 500 }
      );
    }
  ...
  if (pathname === "/api/upload" && request.method === "POST") {
    ...
    if (!env.IMAGES) {
      return json(
        {
          error: "Image storage (R2) isn't set up yet. See README.md."
        },
        { status: 500 }
      );
    }
