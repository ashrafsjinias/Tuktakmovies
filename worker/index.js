...
async function backfillTmdbDetails(
  ...
  for (
    ...
    try {
      const itemRes =
        await fetch(
          `https://api.themoviedb.org/3/${mediaType}/${row.tmdb_id}?api_key=${env.TMDB_API_KEY}`
        );


      if (!itemRes.ok) {
        console.log(
          "TMDB backfill failed for",
          row.tmdb_id
        );
...
async function handleApi(
  ...
  // ==================================================
  // POSTS API WITH UNLIMITED PAGINATION
  // ==================================================
  ...
  if (
    ...
    // Number of pages has NO maximum.
    const limit =
      Number.isFinite(
        requestedLimit
      ) &&
